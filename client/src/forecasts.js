...
import { models, stats } from "./schema";
...
models.normal.beta0 = data => {
	...
	return [a, b, 20, 2, 2 * a, b + 210, 20, 2];
};
...
models.integral.beta0 = data => {
	...
	return [a, b, c, 2, 2 * a, b + 210, c, 2];
};

export function distributions(data, stat, region, city, cb, init) {
	const m = models[stats[stat].model];
	const t = data.map(([t]) => t);
	const beta0 = init || m.beta0(data);
	const func = m.f;
	...
	for(let s = 0; s < rounds; ++s) {
		...
		if(bestSr2 === infinite) {
			cb({ error: "can't find better", region, city, stat, s });
			return beta;
		}
	...
	cb({ error: "too many steps", region, city, stat });
	return beta;
}
